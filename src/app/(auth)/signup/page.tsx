...
export default function SignupPage() {
    ...
    return (
      <div className="relative flex min-h-screen items-center justify-center bg-zinc-950 px-4 overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-sm text-center">
          ...
          <p className="mt-3 text-sm text-zinc-400">Te enviamos un enlace de confirmación a</p>
          <p className="mt-1 text-sm font-medium text-white">{email}</p>

          <Link
            href="/login"
            className="mt-8 inline-block text-sm text-violet-400 transition-colors hover:text-violet-300"
          >
            Volver al login
          </Link>
        </div>
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center bg-zinc-950 px-4 overflow-hidden">
      {/* Animated background */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -top-1/2 -right-1/2 h-full w-full animate-[spin_25s_linear_infinite] opacity-30">
          <div className="absolute top-1/2 left-1/2 h-96 w-96 -translate-x-1/2 -translate-y-1/2 rounded-full bg-violet-600/20 blur-3xl" />
        </div>
        <div className="absolute -bottom-1/2 -left-1/2 h-full w-full animate-[spin_20s_linear_infinite_reverse] opacity-20">
          <div className="absolute top-1/2 left-1/2 h-80 w-80 -translate-x-1/2 -translate-y-1/2 rounded-full bg-violet-500/20 blur-3xl" />
        </div>
      </div>
          ...
          <form onSubmit={handleSignup} className="space-y-5">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    ...
                    required
                    className="block w-full rounded-lg border border-zinc-800 bg-zinc-900 px-4 py-3 text-white placeholder-zinc-600 transition-all duration-200 focus:border-violet-500/50 focus:outline-none focus:ring-2 focus:ring-violet-500/20"
                    placeholder="Tu nombre"
                  />
                  <div
                    className={`absolute inset-0 -z-10 rounded-lg bg-violet-500/10 blur-xl transition-opacity duration-300 ${
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-zinc-600">Mínimo 6 caracteres</p>
              </div>
            </div>
            ...
            <div className="relative py-2">
              ...
              <div className="relative flex justify-center">
                <span className="bg-zinc-900/50 px-3 text-xs text-zinc-600">o</span>
              </div>
            </div>
            ...
            <Button
              type="button"
              variant="outline"
              className="w-full h-12 border-zinc-800 bg-transparent hover:bg-zinc-800/50 transition-all duration-200"
